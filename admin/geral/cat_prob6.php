...
<body>
	...
	<div class="container-fluid">
		<h4 class="my-4"><i class="fas fa-tags text-secondary"></i>&nbsp;<?= TRANS('PROBLEM_TYPES_CATEGORIES'); ?>&nbsp;<span class="badge badge-secondary p-2" id="cat-label"><?= $row_config['conf_prob_tipo_6']; ?>&nbsp;<i class="fas fa-edit"></i></span></h4>
		...
		$query = "SELECT * FROM prob_tipo_6 ";

		if (isset($_GET['cod'])) {
			$query .= " WHERE probt6_cod = ".(int)$_GET['cod']."  ";
		}
        $query .= " ORDER BY probt6_desc";
					...
					<tbody>
						...
						?>
							<tr>
								...
								<td class="line"><button type="button" class="btn btn-secondary btn-sm" onclick="redirect('<?= $_SERVER['PHP_SELF']; ?>?action=edit&cod=<?= $row['probt6_cod']; ?>')"><?= TRANS('BT_EDIT'); ?></button></td>
								<td class="line"><button type="button" class="btn btn-danger btn-sm" onclick="confirmDeleteModal('<?= $row['probt6_cod']; ?>')"><?= TRANS('REMOVE'); ?></button></td>
							</tr>
		...
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && empty($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					<label for="categoria" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('CATEGORY'); ?></label>
					<div class="form-group col-md-10">
						<input type="text" class="form-control " id="categoria" name="categoria" value="<?= $row['probt6_desc']; ?>" required />
                    </div>
		...
		$(function() {
			...
			$('#idSubmit').on('click', function(e) {
				...
				$.ajax({
					...
					method: 'POST',
					data: $('#form').serialize() + '&table=prob_tipo_6&collumnID=probt6_cod&collumnName=probt6_desc',
					dataType: 'json',
		...
		function deleteData(id) {
			...
			$.ajax({
				...
				data: {
					cod: id,
					table: 'prob_tipo_6',
					collumnID: 'probt6_cod',
					collumnName: 'probt6_desc',
					action: 'delete'
				},
				dataType: 'json',
			}).done(function(response) {
		...
		function updateCatLabel() {
            ...
            $.ajax({
                ...
                if (!response.success) {
					...
					if (window.opener) {
						window.opener.loadCatLabel('prob_tipo_6');
					}
                }
            });
            return false;
